...
class DAC:
    ...
    def __init__(self, spi_sck_pn, spi_mosi_pn, spi_miso_pn, spi_cs): # spi_miso_pn dumy, must by any input pin 
        ...
        self.spi = SoftSPI(baudrate=10000, polarity=0, phase=0, bits=8, firstbit=SoftSPI.MSB, sck=sck, mosi=mosi, miso=miso)
    ...
    def min(self):
        self.raw(self.RANGE_MIN_LSB)
        
    def max(self):
        self.raw(self.RANGE_MAX_LSB)
    ...
    def voltage(self,V):
        if self.callib_min.get() == None:
            print('err dac_not_calibrated')
            return        
        if not (self.callib_min.get() <= V <= self.callib_max.get()):
            print('err voltage_out_of_valid_range')
            return
        lsb = int(((V-self.callib_min.get())*self.RANGE_MAX_LSB)/(self.callib_max.get()-self.callib_min.get()))+2
        self.raw(lsb)
